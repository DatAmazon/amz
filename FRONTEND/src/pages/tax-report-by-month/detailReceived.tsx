...
const DetailReceived: React.FC<DataType> = () => {
  ...
  const fetchData = async (values: any) => {
    try {
      ...
      const response = await axios.post('/detail-message-received', {
  ...
  const handleExportExcel = async () => {
    const res = await fetchData({ fromDate, toDate });
    if (res && res.data) {
      await CommonUtils.exportExcel(res.data.data, 'Danh sách thông điệp', 'ListMessage');
    } else {
      console.error('Lỗi tải dữ liệu khi xuất excel');
    }
  };
    ...
    <div>
      ...
      >
        <div style={{ display: 'flex', justifyContent: 'space-between', padding: '10px 5px' }}>
          <div>
            <h3 style={{ margin: 0 }}>3. CHI TIẾT TRẢ VỀ</h3>
          </div>
          <div>
            <Button
              style={{
